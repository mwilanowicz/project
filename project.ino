...
#define Push 7    // Push button pin connected to D10
...
const FrequencySetting frequencyLookupTable[101] = {
    {65535, 0b000}, // 0 Hz setting, Timer stopped (Output toggled off)
    {62499, 0b001}, // 256 Hz
    {31249, 0b001}, // 512 Hz
    {20832, 0b001}, // ~768 Hz
    {15624, 0b001}, // 1024 Hz
...
void adjustFrequency() {
  ...
  // Only update if the read value has changed significantly
  if (abs(freqValue - lastFreqValue) > 5) {
    // Map the potentiometer value from 0-1023 to a frequency index of 0-100(0 Hz)
    // This corresponds to frequencies from 1Hz to 100Hz
    unsigned int frequencyIndex = map(freqValue, 0, 1023, 0, 100);

    // Retrieve the corresponding OCR1A value and prescaler setting from the lookup table
    ...
    // Apply the new OCR1A value
    OCR1A = newOCR1A;

    // Apply the new prescaler setting, ensuring to only modify the prescaler bits
    TCCR1B = (TCCR1B & 0b11111000) | newPrescalerBits;

    // Calculate the actual frequency based on the OCR1A value
    unsigned long frequency = 16000000 / (OCR1A + 1); // +1 because of how timers work -> 16000000 / (624 + 1) = 25600 Hz

    displayFrequency = map(frequency, 256, 25600, 1, 100); 

    tMax = (1000000 / frequency); // Convert frequency to period in miliseconds
    // 1000ms -> 1 Hz
    // 500ms -> 2Hz = 1/500;]
    // T = 1/f => f - 1/T
    calcTMax = map(tMax, 39, 3906, 10, 1000); // Convert period to range from 10ms to 1000ms

    Serial.println(calcTMax);
    // Serial.println(tMax);
    ...
    // Serial.print(", OCR1A: ");
    // Serial.print(newOCR1A);
    // Serial.print(", Prescaler Bits: 0b");
    // Serial.println(newPrescalerBits, BIN);
    //     // Optional: If you want to print the actual prescaler value (not just the bits)
    // unsigned long prescalerValue = 0;
    // switch(newPrescalerBits) {
    //   case 0b001: prescalerValue = 1; break; // No prescaling
    //   case 0b010: prescalerValue = 8; break; // Prescale by 8
    //   case 0b011: prescalerValue = 64; break; // Prescale by 64
    //   case 0b100: prescalerValue = 256; break; // Prescale by 256
    //   case 0b101: prescalerValue = 1024; break; // Prescale by 1024
    //   default: prescalerValue = 0; // Unknown or not set
    // }
    // Serial.print("Actual Prescaler Value: ");
...
void loop() {
  adjustAmplitude();
  adjustPWM(); // Regulacja szerokości impulsu PWM na podstawie wartości z potencjometru
  adjustFrequency(); // Dostosowanie częstotliwości na podstawie odczytu z potencjometru
   
  unsigned int pulseWidth = map(settingsValue[1], 0, 100, 0, calcTMax - 10);

  unsigned int pulseDelay = map(settingsValue[2], 0, 100, 0, (calcTMax - 7 - pulseWidth));
